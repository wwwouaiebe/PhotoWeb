    ...
    */
	...
	#extractDate ( ) {
		...
		if ( dateTime ) {
			...
			// Searching file name
			let strDateTime = this.#photoSrcFileName.split ( '/' ).reverse ( ) [ 0 ].split ( '.' ) [ 0 ];
			...
			if ( isNaN ( Date.parse ( strDateTime ) ) ) {
				this.#noExifPhotos.push ( this.#photoSrcFileName );
			}
			else {
	...
	#extractPhotoDimensions ( ) {
		...
		if ( 0 === this.#photoWidth || 0 === this.#photoHeight ) {
			this.#noWidthHeightPhotos.push ( this.#photoSrcFileName );
		}
	...
	#addPost ( ) {
		const post = new Post ( );
		post.photoSrcFileName = this.#photoSrcFileName;
		...
		post.categories = this.#categories.slice ( );
		...
		for ( let filesCounter = 0; filesCounter < fileNames.length; filesCounter ++ ) {
			...
			if ( lstat.isDirectory ( ) ) {
				...
				if (
					...
					this.#photoSrcFileName = srcDir + this.#photoIsoDate.replaceAll ( ':', '' ) + '.jpg';
					fs.renameSync ( srcDir + fileName, this.#photoSrcFileName );
				}
